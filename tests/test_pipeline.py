...
class _FakeClassifier:
    def __init__(
        self, intent_by_text: dict[str, Intent], failing_texts: frozenset[str] = frozenset()
    ) -> None:
    ...
    def classify(self, message: Message) -> ClassificationResult:
        ...
        return ClassificationResult(
            intent=self._intent_by_text[message.text], confidence=0.9, rationale="fake"
        )
...
def _conversation(conversation_id: str, customer_text: str, brand_text: str) -> Conversation:
    ...
    return Conversation(
        ...
        messages=[
            ...
            Message(
                id=f"{conversation_id}-b",
                author_id="SpotifyCares",
                text=brand_text,
                created_at=now,
                in_reply_to_id=f"{conversation_id}-c",
            ),
        ],
    )
...
def test_build_precedent_index_groups_by_intent() -> None:
    ...
    classifier = _FakeClassifier(
        {"my app crashed": Intent.PLAYBACK_BUG, "cancel my plan": Intent.CANCELLATION_RETENTION}
    )
    settings = Settings()
    ...
    assert (
        index.by_intent[Intent.PLAYBACK_BUG][0].resolution
        == "sorry about that, please update the app"
    )
